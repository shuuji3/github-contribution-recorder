...
export function fetch(
  ...
  try {
    ...
    return items.filter((item: any) => {
      ...
      return (
        filter(item) &&
        isMyActivity(item, username) &&
        (date ? isAfterDate(date, userConfig.sinceDate) : true)
      )
    })
  } catch (e) {
...
export function getRepos(username: string) {
  ...
  while (true) {
    const perPage = 100
    const output = execSync(`gh api user/repos?sort=updated&per_page=${perPage}&page=${page}`, {
      encoding: 'utf8',
    })
